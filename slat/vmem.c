...
void setup_spt(pte_t pt[9][PTECOUNT], unsigned long paddr_code_base, unsigned long paddr_data_base, unsigned int levels)
{
  ...
  pt[0][PTECOUNT - 1] = ((pte_t)pt[spp] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;

  // Ptr pages
  for (int i = upp; i < ulp; i++)
    pt[i][0] = ((pte_t)pt[i + 1] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;
  for (int i = spp; i < slp; i++)
    pt[i][PTECOUNT - 1] = ((pte_t)pt[i+1] >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;

  // Leaf pages
  pt[ulp][0] = ((pte_t)paddr_code_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_UCODE;
  pt[ulp][1] = ((pte_t)paddr_data_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_UDATA;
  pt[slp][PTECOUNT - 1] = ((pte_t)paddr_code_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_SCODE;
  pt[slp][PTECOUNT - 2] = ((pte_t)paddr_data_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_SDATA;
}
...
void setup_vspt(pte_t pt[9][PTECOUNT], unsigned long paddr_base, unsigned int levels)
{
  ...
  // Root page
  pt[rp][0] = (C_SPA2GPA_SLAT((pte_t)pt[upp]) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;
  pt[rp][PTECOUNT - 1] = (C_SPA2GPA_SLAT((pte_t)pt[spp]) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;

  // Ptr pages
  for (int i = upp; i < ulp; i++)
    pt[i][0] = (C_SPA2GPA_SLAT((pte_t)pt[i+1]) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;
  for (int i = spp; i < slp; i++)
    pt[i][PTECOUNT - 1] = (C_SPA2GPA_SLAT((pte_t)pt[i+1]) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_PTR;

  // Leaf pages
  pt[ulp][0] = (C_SPA2GPA_VCODE((pte_t)paddr_base) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_UCODE;
  pt[ulp][1] = (C_SPA2GPA_VDATA((pte_t)paddr_base) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_UDATA;
  pt[slp][PTECOUNT - 1] = (C_SPA2GPA_VCODE((pte_t)paddr_base) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_SCODE;
  pt[slp][PTECOUNT - 2] = (C_SPA2GPA_VDATA((pte_t)paddr_base) >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_SDATA;
}
...
void setup_gpt(pte_t pt[3][PTECOUNT], unsigned long paddr_code_base, unsigned long paddr_data_base, unsigned long paddr_slat_base)
{
  ...
  pt[1][1] = ((pte_t)paddr_slat_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_SLAT;
  pt[2][0] = ((pte_t)paddr_code_base >> RISCV_PGSHIFT << PTE_PPN_SHIFT) | PTE_BITS_VCODE;
